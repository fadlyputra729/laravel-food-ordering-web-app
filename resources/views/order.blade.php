...
@section('content')
  <h1 class="px-4 pt-1 pb-3 text-3xl font-bold">
    <div class="flex flex-row flex-1">
      <span class="mr-5 self-center"> History Transaksi</span>
        ...
        <div class="px-3 py-2">
          <div class="flex flex-row p-4 leading-normal border shadow-md hover:bg-gray-100">
            <div class="p-2 w-1/5">
              ...
              <p class="mb-1"> Status Order: <span class="font-semibold capitalize"> {{ $order->status }} </span></p>
              <p class="mb-1"> Status Pembayaran: <span
              ...
              <div class="mt-2">
                @if(isset($order->url_pembayaran) && !in_array($order->status_pembayaran, ['success', 'expired']))
                  <a href="{{ $order->url_pembayaran }}"
                     class="font-semibold btn btn-primary mr-2">
                    <span> Bayar Sekarang </span>
                  </a>
                @endif
                @if(isset($order->status_pembayaran) && !in_array($order->status_pembayaran, ['success']))
                  <button onclick="remove_form_action({{$order->id}})" type="button"
            ...
            <div class="w-4/5">
              <div class="flex flex-col justify-between">
                @foreach ($order->food as $food)
                  ...
                  <div
                    class="flex flex-col items-center -white rounded-lg border shadow-md md:flex-row md:max-w-full hover:bg-gray-100">
                    <div class="flex rounded-lg">
                      <img class="flex h-28 w-44 object-fill rounded-lg" src="{{ asset($food->picture) }}" alt="">
                    </div>
                    <div class="flex flex-col place-content-center px-4 py-3 leading-normal w-4/6">
                      ...
                      <p class="flex font-normal text-gray-700"> Harga: <b>&nbsp;Rp.
                          {{ number_format($food->price*$food->pivot->quantity) }} &ensp;</b> <span class="opacity-60"> [Rp. {{ number_format($food->price) }} per unit] <span>
                      </p>
                    </div>
                  </div>
                  <div class="p-1"></div>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="p-1 sm:flex sm:items-start">
                <div
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="-3.5 -3 31 31"
                ...
                <div class="pl-1 sm:mt-0 sm:ml-4 sm:text-left">
                  <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Apa kamu yakin ingin
                    membatalkan & menghapus pesanan ini dari history transaksi?</h3>
                </div>
              </div>
            </div>
  ...
  @else
    <p class="px-4 pt-4 text-lg">
      Silahkan login terlebih dahulu untuk melihat history transaksi.
    </p>
  @endif
