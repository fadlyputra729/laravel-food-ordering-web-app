                ...
                <div class="flex flex-col justify-between">
                    <div class="flex flex-col items-center bg-white rounded-lg border shadow-md md:flex-row md:max-w-full hover:bg-gray-100">
                        ...
                        <div class="flex flex-col place-content-center px-4 py-3 leading-normal w-4/6">
                            ...
                            <p class="flex font-normal text-gray-700 quantity">
                        ...
                        <div class="flex justify-center leading-normal w-1/6">
                            <button onclick="remove_form_action({{ $food['id'] }})" type="button"
                                class="openRemoveModal text-red-700 font-semibold bg-inherit border-red-500 rounded hover:text-white hover:bg-red-500 hover:border-transparent py-1 px-3 border-2">
                                <span> Hapus </span>
                            </button>
                        </div>
                    </div>
...
@endforeach
    ...
    <!-- Remove item modal -->
    <div class="invisible flex h-screen overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-4 z-50 justify-center items-center md:inset-0 h-modal sm:h-full"
        id="remove-modal">
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="p-1 sm:flex sm:items-start">
                        ...
                        <div class="pl-1 sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Apa kamu yakin
                                menghapus pesanan ini?</h3>
                            {{-- <div class="mt-2">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <form name="remove_form" id="remove_form" method="POST">
                        ...
                        @method('delete')
                        <button type="submit"
                            class="closeRemoveModal w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                            Konfirmasi </button>
                    </form>
                    <button type="button"
                        class="closeRemoveModal w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-inherit text-base font-medium text-gray-700 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm">
                        Batal </button>
                </div>
            </div>
        </div>
    ...
    {{-- Place Order Modal --}}
     <div x-show="show" tabindex="0"
        class="invisible z-40 overflow-auto left-0 top-8 bottom-0 right-0 w-full h-full fixed" id="order-modal">
        <div class="z-50 relative p-3 mx-auto my-0 max-w-full" style="width: 600px;">
            <div class="bg-white rounded shadow-lg border flex flex-col overflow-hidden">
                <button
                    class="closeOrderModal fill-current h-6 w-6 absolute right-0 top-0 m-6 font-3xl font-bold">&times;</button>
                <div class="px-6 py-3 text-xl border-b font-bold">Place Order</div>
                <form method="POST" action="/cart/placeorder" class="px-4 pt-2 pb-4" name="place-order-form"
                    id="place-order-form">
                    @csrf
                    <div class="p-6 flex-grow">
                        <div class="mb-4">
                            ...
                            <div class="columns-2 gap-2 w-fit">
                                <div><label> <input type="radio" name="type" id="pickupType" value="pickup"
                                            onclick="showAddressField()" checked> Pickup </label></div>
                                <div><label> <input type="radio" name="type" id="deliveryType" value="delivery"
                                            onclick="showAddressField()"> Delivery </label></div>
                            </div>
                        </div>
                        <div class="hidden" id="address_div">
                            ...
                            @if (Auth::user())
                                <textarea rows="3"
                                    class="resize-none shadow-sm appearance-none border border-red-500 rounded w-full py-2 px-3 text-gray-700 mb-1 leading-tight focus:outline-none focus:shadow-outline"
    ...
    <div class="invisible flex h-screen overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-0 z-50 justify-center items-center md:inset-0 h-modal sm:h-full"
        id="payment-modal">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
        <div class="relative px-4 w-full max-w-md h-full md:h-auto">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-6 pt-3 text-center">
                    ...
                    <div class="flex justify-center pb-5">
                        <svg role="status" class="mr-2 w-16 h-16 text-gray-200 animate-spin fill-green-500"
                    ...
                    <h3 class="mb-3 text-lg font-normal text-gray-500">Pembayaran sedang di proses.</h3>
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-6 pt-3 text-center">
                    ...
                    <div class="flex justify-center pb-4">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-20 w-20 stroke-green-600" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <h3 class="mb-4 text-2xl font-semibold text-gray-600">Pembayaran Sukses!</h3>
                    {{-- <button data-modal-toggle="popup-modal" type="button" class="closePaymentModal text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                        Yes, I'm sure
                    </button>
                    <button data-modal-toggle="popup-modal" type="button" class="closePaymentModal text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600">No, cancel</button> --}}
