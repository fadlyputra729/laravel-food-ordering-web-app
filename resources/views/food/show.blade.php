...
@section('content')
  <div class="px-3 py-2 flex flex-col justify-center items-center h-[70vh]">
    <div
      class="flex flex-row leading-normal border shadow-md rounded-lg max-w-3xl min-w-[50%] bg-white max-h-[96%] min-h-[50%]">
      ...
      <div class="flex flex-col p-4 h-full flex-grow">
        <div class="flex-grow flex flex-col justify-around">
          ...
          <h1 class="font-semibold font-sans text-lg text-red-500 leading-8">
            Rp. {{ number_format($food['price']) }}</h1>
        ...
        <div class="pt-3 flex flex-col flex-grow justify-around">
          ...
          <button id='addCartBtn' {{ $food['stock'] <= 0 ? 'disabled' : '' }}
                  class="btn btn-{{ $food['stock'] <= 0 ? 'secondary' : 'primary' }} mt-3">
            {{ $food['stock'] <= 0 ? 'Stok Kosong' : 'Tambahkan Ke Keranjang' }}
          </button>
        </div>
      </div>
    </div>
